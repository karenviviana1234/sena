...
import EtapaModal from '../templates/TempleteEtapa.jsx';
...
import EtapaContext from '../context/EtapaContext.jsx';

export function EtapaPracticaPage() {
    ...
    function EjemploEtapa() {
        ...
        const filteredItems = React.useMemo(() => {
          let filteredeatapa = etapas;
          if (hasSearchFilter) {
              filteredeatapa = filteredeatapa.filter(etapa =>
                  String(etapa.id_asignacion).toLowerCase().includes(filterValue.toLowerCase()) ||
                  etapa.fecha_inicio.toLowerCase().includes(filterValue.toLowerCase()) ||
                  etapa.fecha_fin.toLowerCase().includes(filterValue.toLowerCase()) ||
                  etapa.productiva.toLowerCase().includes(filterValue.toLowerCase()) ||
                  etapa.instructor.toLowerCase().includes(filterValue.toLowerCase())
              );
          }
          // Asegúrate de que statusFilter se maneje correctamente
          if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
              filteredeatapa = filteredeatapa.filter(etapa =>
                  Array.from(statusFilter).includes(etapa.estado)
              );
          }
          return filteredeatapa;
      }, [etapas, filterValue, statusFilter]);
        ...
        const renderCell = React.useCallback((etapa, columnKey) => {
            const cellValue = etapa[columnKey];

            const handleUpdateClick = (identificacion, etapa) => {
                localStorage.setItem('idUser', identificacion);
                clickEditar(identificacion, etapa);
            };

            switch (columnKey) {
                case "estado":
                    return (
                        <Chip className="capitalize" color={statusColorMap[etapa.estado]} size="sm" variant="flat">
                            {cellValue}
                        </Chip>
                    );
                case "actions":
                        ...
                        <div className="relative flex justify-start  items-center gap-2">
                            <Dropdown>
                                <div className="flex items-center gap-2">
                                    <ButtonActualizar onClick={() => handleToggle('update', setEtapaId(etapa))} />
                                    <ButtonDesactivar
                                        onClick={() => peticionDesactivar(etapa.id_asignacion)}
                                        estado={etapa.estado}
                                    />
                                </div>
                            </Dropdown>
                ...
                <>
                    <div className="flex flex-col  mt-3" >
                        ...
                        <div className="flex justify-between items-center z-10 mr-30  mt-2">
                            <span className="text-white text-small">Total {etapas.length} Resultados</span>
    ...
    const {idEtapa, setEtapaId } = useContext(EtapaContext)
    ...
    const peticionGet = async () => {
        try {
            await axiosClient.get('/asignacion/listar').then((response) => {
        ...
        try {
            axiosClient.put(`/usuario/desactivarUsuario/${identificacion}`, null).then((response) => {
                if (response.status == 200) {
                    ...
                    Swal.fire({
                        title: "¿Estás seguro?",
                        text: "¡Esto podra afectar a tus asignaciones!",
                        icon: "question",
    ...
    const handleSubmit = async (formData, e) => {
        ...
        try {
            ...
            if (mode === 'create') {
                ...
                await axiosClient.put(`/asignacion/actualizar/${idEtapa.id_asignacion}`, formData).then((response) => {
                    if (response.status === 200) {
                        Swal.fire({
                            position: "center",
                            icon: "success",
            ...
            <div>
                ...
                <div className='w-full max-w-[90%] ml-28 items-center p-10'>
                    ...
                    <EtapaModal
                        ...
                        title={mode === 'create' ? 'Registrar asignacion' : 'Actualizar asignacion'}
